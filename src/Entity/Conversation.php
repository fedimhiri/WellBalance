...
class Conversation
{
    ...
    public function getLastMessage(): ?Message { return $this->messages->last() ?: null; }

    public function getUnreadCountFor(User $user): int
    {
        $count = 0;
        foreach ($this->messages as $message) {
            if ($message->getSender() === $user) continue;
            if (!$message->isRead()) $count++;
        }
        return $count;
    }

    public function getMedecinId(): int { return $this->medecin ? $this->medecin->getId() : 0; }
    public function getUserId(): int { return $this->user ? $this->user->getId() : 0; }
}
